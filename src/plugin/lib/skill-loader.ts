...
export function scoreSkillCandidates(text: string, agent?: string): SkillScoreBreakdown[] {
  ...
  for (const skill of candidates) {
    ...
    if (skill === "java-kotlin" && /\.kt\b|kotlin/i.test(lower) && [...candidates].some((candidate) => candidate.startsWith("android-"))) contributions.push({ source: "file", score: 40, reason: "kotlin source keeps JVM language guidance with Android skills" });
...
export function getSubAgentSkillProfile(agent: string, prompt: string): string[] {
  ...
  if (agent === "explorer") return boost(prioritizeSkills([...new Set(["codebase-intelligence", "code-archaeology", ...weighted.filter((skill) => ["codebase-intelligence", "code-archaeology", "software-engineering"].includes(skill))])])).slice(0, MAX_SUBAGENT_SKILLS);
  return [];
}
...
async function readSkillFile(skillName: string): Promise<string | null> {
  ...
  try {
    ...
    const scan = scanSkillContent(skillName, content);
    if (scan.blocked) {
      lastBlockedSkills.push({
        name: skillName,
        riskScore: scan.riskScore,
        reason: scan.signals.map((signal) => signal.message).join(" "),
      });
      return null;
    }
...
const SKILL_ELIGIBLE_AGENTS = new Set(["oracle", "frontend", "jce-researcher", "android", "explorer"]);
